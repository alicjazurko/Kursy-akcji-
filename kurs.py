...
frameDataInputs.grid(row=2, column=1)
...
def show_multiples(tic):
    ...
    # stworzenie tabelki do tkinter
    cols = list(mdf.columns)
    table = ttk.Treeview(frameData)
    table.grid(row=1, column=1)
    table["columns"] = cols

    for i in cols:
        table.column(i, anchor="w")
        table.heading(i, text=i, anchor="w")

    for index, row in mdf.iterrows():
        table.insert("", 0, text=index, values=list(row))
...
def show_plots(year=2021, month=1, day=1, tic="MSFT"):
    ...
    fig1 = plt.figure(figsize=(9, 7))  # rozmiar wykresu
